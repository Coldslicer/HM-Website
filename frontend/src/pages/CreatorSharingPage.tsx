...
import {ChannelData, DatabaseCreator} from "../types";

export const CreatorSharingPage: React.FC = () => {
    const {campaignId} = useParams<{ campaignId: string }>();


    const [creators, setCreators] = useState<DatabaseCreator[]>([]);
    ...
    const fetchCreators = async (campaignId: string) => {
        ...
        const creatorsWithChannelData: (DatabaseCreator & ChannelData)[] = await Promise.all(
        ...
        const sortedCreators = creatorsWithChannelData.sort(
            (a, b) => (b.selected === a.selected ? 0 : (b.selected ? -1 : 1)),
        );
        setCreators(sortedCreators);

        const selected = sortedCreators.filter((c) => c.selected);
        setTotalRate(selected.reduce((acc, c) => acc + (c.rate ?? 0), 0));
        setTotalRateCPM(
            selected.reduce(
    ...
    const handleSelectCreator = async (creator: DatabaseCreator) => {
        ...
        setTotalRateCPM(
            selected.reduce(
                (acc, c) => acc + ((c.rate_cpm ?? 0) * (c.averageViews || 0)) / 1000,
                0,
            ),
        );
            ...
            <div className="overflow-x-auto rounded-lg shadow-md mb-4">
                <table className="min-w-full bg-white border-collapse rounded-lg">
                    ...
                    {creators.map((creator) => (
                        <tr
                            key={creator.id}
                            onClick={() => handleSelectCreator(creator as DatabaseCreator)}
