...
export const CreatorTimeline = () => {
  ...
  const fetchSelectedCreators = async () => {
    ...
    const sortedCreators = creatorsWithHandle.sort((a, b) => {
      ...
      if (!isCompleted(a.draft) && isCompleted(b.draft)) return 1;
      if (isCompleted(a.contract_signed) && !isCompleted(b.contract_signed))
        return -1;
      if (!isCompleted(a.contract_signed) && isCompleted(b.contract_signed))
        return 1;
      return 0;
    ...
    return (
      ...
      >
        ...
        <div className="flex justify-between items-center mt-6">
          <div
            className={`relative flex flex-col items-center w-1/4 p-4 rounded-md border-2 ${
              contractComplete ? "border-green-500" : "border-gray-300"
            } bg-gray-50`}
          >
            <div className="text-sm text-gray-600">Contract Signed</div>
            <div
              className={`mt-2 text-sm cursor-pointer ${
                contractComplete ? "text-green-500 underline" : "text-gray-500"
              }`}
              onClick={() =>
          ...
          <div
            className={`w-1/6 h-1 ${
              contractComplete && draftComplete ? "bg-green-500" : "bg-gray-300"
            }`}
          ></div>
          <div
            className={`relative flex flex-col items-center w-1/4 p-4 rounded-md border-2 ${
              draftComplete ? "border-green-500" : "border-gray-300"
            } bg-gray-50`}
          >
            <div className="text-sm text-gray-600">Draft Submitted</div>
            <div
              className={`mt-2 text-sm cursor-pointer ${
                draftComplete ? "text-green-500 underline" : "text-gray-500"
              }`}
          ...
          <div
            className={`w-1/6 h-1 ${
              draftComplete && finalComplete ? "bg-green-500" : "bg-gray-300"
            }`}
          ></div>
          <div
            className={`relative flex flex-col items-center w-1/4 p-4 rounded-md border-2 ${
              finalComplete ? "border-green-500" : "border-gray-300"
            } bg-gray-50`}
          >
            <div className="text-sm text-gray-600">Live Submitted</div>
            <div
              className={`mt-2 text-sm cursor-pointer ${
                finalComplete ? "text-green-500 underline" : "text-gray-500"
              }`}
