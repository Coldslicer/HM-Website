...
import { SUPABASE_CLIENT, DISCORD_CLIENT } from '../util/clients.js';
...
export async function validateDiscordId(discordId) {
  ...
  try {
    ...
    return { valid: true, username: user.username, discriminator: user.discriminator };
  } catch (error) {
  ...
  for (const creator of unselectedCreators) {
    if (creator.channel_id) {
      try {
        ...
        if (channel) await channel.delete('Removing unselected creator channel');
      } catch {
...
export async function purgeCategory(campaignId) {
  ...
  const category = await guild.channels.fetch(campaign.category_id).catch(() => null);
...
export async function addCreatorToDiscord(creatorId) {
  ...
  const { data: campaign, error: campaignError } = await SUPABASE_CLIENT
    .from('campaigns')
    .select('id, company_name, rep_name, category_id, server_id, client_id, group_chat_channel_id')
  ...
  if (campaign.group_chat_channel_id) {
    const groupChannel = await guild.channels.fetch(campaign.group_chat_channel_id).catch(() => null);
...
export async function removeCreatorFromDiscord(creatorId) {
  ...
  if (campaign.group_chat_channel_id) {
    const groupChannel = await guild.channels.fetch(campaign.group_chat_channel_id).catch(() => null);
    if (groupChannel?.isTextBased()) {
      await groupChannel.permissionOverwrites.delete(creator.discord_id).catch(() => null);
    }
  }
...
export async function createGroupChat(campaignId) {
  ...
  const { data: creators, error: creatorsError } = await SUPABASE_CLIENT
    ...
    .eq('campaign_id', campaignId)
    .eq('selected', true);
  if (creatorsError || !creators.length) throw new Error('No selected creators found');
  ...
  for (const c of creators) {
    try {
      const user = await DISCORD_CLIENT.users.fetch(c.discord_id);
